...
module Scarpe::Wasm
  ...
  class TextDrawable < Drawable
    ...
    # Calzini expects and then let it render. See Wasm::Para
    ...
    def items_to_display_children(items)
      ...
      items.map do |item|
        if item.is_a?(String)
          ...
          Scarpe::Wasm::DisplayService.instance.query_display_drawable_for(item)
        end
      end
    end
    ...
    def html_element
      @elt_wrangler ||= Scarpe::Wasm::WebWrangler::ElementWrangler.new(selector: %{document.getElementsByClassName("id_#{html_id}")}, multi: true)
    end
  end
